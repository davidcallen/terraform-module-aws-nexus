...
# ---------------------------------------------------------------------------------------------------------------------
resource "aws_acm_certificate" "nexus-amazon-provider" {
...
resource "aws_lb_listener_certificate" "nexus-public" {
...
# Note that the aws_lb_listener is defined outside this module so can be shared
resource "aws_lb_listener_rule" "nexus-public-https" {
  count        = (var.ha_high_availability_enabled && var.ha_public_load_balancer.enabled) ? 1 : 0
  listener_arn = var.ha_public_load_balancer.alb_listener_arn
  priority     = var.ha_public_load_balancer.alb_listener_priority
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.nexus-public-https[count.index].arn
  }
  condition {
    host_header {
...
resource "aws_lb_target_group" "nexus-public-https" {
  count                = (var.ha_high_availability_enabled && var.ha_public_load_balancer.enabled) ? 1 : 0
...
resource "aws_lb_listener_certificate" "nexus-private" {
...
# Note that the aws_lb_listener is defined outside this module so can be shared
resource "aws_lb_listener_rule" "nexus-private-https" {
  count = (var.ha_high_availability_enabled && var.ha_private_load_balancer.enabled) ? 1 : 0
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.nexus-private-https[count.index].arn
  }
  condition {
    host_header {
      values = [var.hostname_fqdn]
...
resource "aws_lb_target_group" "nexus-private-https" {
  count = (var.ha_high_availability_enabled && var.ha_private_load_balancer.enabled) ? 1 : 0
