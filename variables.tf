...
variable "disk_nexus_home" {
  description = "Disk storage options for Nexus Home Dir. Will be mounted at /mnt/nexus and symlinked to /var/nexus"
  type = object({
    enabled = bool
    type    = string
    # EBS or EFS
...
//  default = []
//}
variable "nexus_admin_user_password_secret_id" {
  description = "AWS Secrets ID for password to login to Nexus application as 'admin' user."
  type        = string
  default     = ""
}
variable "nexus_jenkins_user_password_secret_id" {
  description = "AWS Secrets ID for password to login to Nexus application as 'jenkins' user."
  type        = string
  default     = ""
}
